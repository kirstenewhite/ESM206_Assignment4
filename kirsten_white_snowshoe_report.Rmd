...
The total number of juvenile snowshoe hares trapped during each year of the study (1998 - 2012) are counted and visualized. 
```{r}
### Data Wrangling 
...
In future analysis, data can be standardized by having equal sample sizes.



##  **Part 2: Juvenile Hare Weights**
Juvenile snowshoe hare weights are compared by sex and site.  
...
ggplot(data = juvenile_weights, aes(x = sex, y = weight, color = grid_full))+
  ...
  scale_color_manual(values = c("skyblue", "goldenrod2", "coral"))+
   labs(x = "Sex", 
       y = "Weight (g)", 
       color = "Site",
...
**Figure 2**: Female and male juvenile hare weights (in grams) are visualized by sex (female, male, and NA) and site, including: Mature (blue), Riparian (orange), and Bonbs (coral). Each point indicates an individual observation for a juvenile hare weight.
...
juvenile_weights_summary <- juvenile_weights %>%
  ...
  kbl(caption = "Table 2: Summary statistics for male and female juvenile weights (g) by site", col.names = c("Site", "Sex","Sample Size", "Mean (g)", "Median (g)", "Min (g)", "Max (g)", "Standard Deviation (g)"), digits = 1) %>%
  kable_classic_2()
...
**Analysis** - A large disparity in sample sizes is evident between each site.  Counts from the Bonanza Riparian site had a significantly higher sample size (242 hares) compared to the other two sites (82 and 47 hares). Central tendency amongst female hares is generally lower than males as seen in the spread of means. 

ADD TABLE WITH WEIGHT COMPARISONS ONLY FOR FEMALE AND MALE TOTALS 
...
juvenile_weights_m_f <- juvenile_weights %>%
  ...
  summarize(sample_size = n(), 
            ...
            max_weight = max(weight, na.rm = TRUE),
            sd_weight = sd(weight, na.rm = TRUE)) %>%
  kbl(caption = "Table 3: Summary statistics for male and female juvenile weights (g)", col.names = c("Sex","Sample Size", "Mean (g)", "Median (g)", "Min (g)", "Max (g)", "Standard Deviation (g)"), digits = 1) %>%
  kable_classic_2()

juvenile_weights_m_f
```

On average, male juvenile hares weigh more than female juvenile hares.The actual difference in means is `r round(male_mean - female_mean, 1)` (g) and the percent difference is `r round((male_mean - female_mean)/((male_mean + female_mean)/2) * 100, 1)`%. The difference in means was found to be statistically significant (Welch’s two-sample t-test:  t(`r round(weight_t_test$parameter,1)`) = `r round(weight_t_test$statistic, 1)`,  p < 0.05) with a small effect size (Cohen’s d = `r round(weight_cohen$estimate, 2)`). Therefore, further investigation is needed to identify whether male snowshoe hares have an actual significant difference in weight compared to female snowshoe hares. 
...
- The **R^2^ value** of `r round(hfoot_weight_lm_out$r.squared, 1)` means that 30% of the variance in hind foot length length is explained by weight. The Pearson's r correlation is `r round(hfoot_weight_cor$estimate, 1)`. This R^2^ value indicates that hind foot length has a medium to moderate influence on hare weight. 
...
Exploratory data analysis reveals the following: 
  ...
  - Male juvenile hares weigh more than female juvenile hares. 
  - A moderate correlation was detected between weight and hind foot length. 
