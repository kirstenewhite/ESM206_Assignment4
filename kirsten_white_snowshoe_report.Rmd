...
ggplot(data = juvenile_trappings, aes(x = year, y = n)) +
  ...
  labs(x = "Year", 
       ...
       caption = "Figure 1: Total number of juvenile hares trapped per year from 1998 through 2012") + 
  theme_grey() + 
...
juvenile_trappings_summary <- juvenile_trappings %>%
  group_by(age) %>%
  summarize(min(juvenile_trappings$n), 
            max(juvenile_trappings$n), 
            mean(juvenile_trappings$n), 
            median(juvenile_trappings$n)) %>%
  kbl(caption = "Table 1: Summary statistics for juvenile hare counts from 1998 - 2012", col.names = c("Age", "min", "max", "mean", "median")) %>%
  kable_minimal()
...
```{r}
ggplot(data = juvenile_weights, aes(x = sex, y = weight, color = grid))+
  geom_jitter(width = 0.1, size = 0.9)+
  facet_wrap(~ grid) +
   labs(x = "Sex", 
       y = "Weight", 
       color = "Site",
